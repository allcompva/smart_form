  ...
  <v-container v-if="desserts != null && desserts != 'undefined'">
    <v-row>
      <v-col cols="12" style="text-align: right">
        <v-btn @click="nuevaPregunta()" style="background-color: #1c3e89 !important; color: white">NUEVA PREGUNTA
        </v-btn>
      </v-col>
    </v-row>
    <v-row>
        ...
        <v-card>
          ...
          <v-card-text>
            <table style="width: 100%">
              <Draggable :list="ficha.lstPreguntas" tag="tbody" @end="saveorden()">
                  ...
                  <td>
                    <v-menu top :close-on-content-click="closeOnContentClick">
                      ...
                      <v-list>
                        ...
                        <v-list-item v-if="item.tipo_pregunta == 1 || item.tipo_pregunta == 2">
                          <v-btn text :href="'/FichasRespuestas/' + item.id + '/' + ficha.id">
      ...
      <template v-slot:default="dialogAdd">
        <v-card>
          ...
          <v-card-text style="margin-top: 20px">
            <v-row>
              <v-col cols="8">
                <v-text-field v-model="nombrePregunta" :rules="[rules.required]" label="Texto Pregunta">
                </v-text-field>
              </v-col>
              <v-col cols="4">
                <v-select v-model="select" :hint="`${select.tipo_pregunta}`" :items="items" item-text="text"
                  item-value="value" label="Tipo Pregunta" persistent-hint return-object single-line>
                </v-select>
              </v-col>
            </v-row>
          </v-card-text>
...
export default {
  ...
  methods: {
    async activa_desactiva(id, estado) {
      ...
      this.ficha = (await this.$http.post("/Fichas/updateActivaPregunta", post)).data;
      this.desserts = this.ficha.lstPreguntas;
      console.log(this.ficha);
    },
    nuevaPregunta() {
