            ...
            <v-col cols="12">
                <v-card>
                    ...
                    <v-data-table :headers="headers" :items="informe" :search="search">
                        ...
                        <template v-slot:[`item.eit`]="{ item }">
                            <span v-html="item.eit"></span>
                        </template>                        
...
export default {
    ...
    async mounted() {
        ...
        try {
            ...
            this.informe = (
                await this.$http.get("/FichasRelevamientos/informeInscriptosGeneral?idFicha=" + vector[0])
            ).data;
            this.smartform = vector[1];
            var d = new Date(vector[2]);
